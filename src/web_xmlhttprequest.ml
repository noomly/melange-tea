...
type __xmlhttprequest

external create : unit -> __xmlhttprequest = "XMLHttpRequest" [@@mel.new]
external abort : unit -> unit

class type _xmlhttprequest = object
  (* Methods *)
  method abort : (unit -> unit[@mel.meth])
  method getAllResponseHeaders : (unit -> string Js.null[@mel.meth])
  method getResponseHeader : (string -> string Js.null[@mel.meth])

  method _open :
    (string -> string -> bool -> string -> string -> unit[@mel.meth])

  method overrideMimeType : (string -> unit[@mel.meth])
  method send : (unit -> unit[@mel.meth])
  method send__string : (string Js.null -> unit[@mel.meth])
  method send__formdata : (Web_formdata.t -> unit[@mel.meth])
  method send__document : (Web_document.t -> unit[@mel.meth])

  (* method send_blob : Web_blob.t -> unit *)
  (* method send_arrayBufferView : Web_arraybuffer_view.t -> unit *)
  method setRequestHeader : (string -> string -> unit[@mel.meth])
