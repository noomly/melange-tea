module Decoder = struct
    ...
    Decoder
      (fun value ->
        ...
        | JSONObject o -> (
            ...
            with ParseFail e ->
              Tea_result.Error ("Invalid keyValuePair parsing: " ^ e))
  ...
  let map2 mapper (Decoder decoder1) (Decoder decoder2) =
    Decoder
      (fun value ->
        let open Tea_result in
        match (decoder1 value, decoder2 value) with
  ...
  let map3 mapper (Decoder decoder1) (Decoder decoder2) (Decoder decoder3) =
    Decoder
      (fun value ->
        let open Tea_result in
        match (decoder1 value, decoder2 value, decoder3 value) with
        | Ok v1, Ok v2, Ok v3 -> Ok (mapper v1 v2 v3)
        | e1, e2, e3 -> (
            let open! Tea_result in
            match e1 |> first e2 |> first e3 with
            | Ok _ -> failwith "Impossible case"
            | Error e -> Error ("map3 -> " ^ e)))

  let map4 mapper (Decoder decoder1) (Decoder decoder2) (Decoder decoder3)
      (Decoder decoder4) =
    Decoder
      (fun value ->
        let open Tea_result in
        match
          (decoder1 value, decoder2 value, decoder3 value, decoder4 value)
        with
        | Ok v1, Ok v2, Ok v3, Ok v4 -> Ok (mapper v1 v2 v3 v4)
        | e1, e2, e3, e4 -> (
            let open! Tea_result in
            match e1 |> first e2 |> first e3 |> first e4 with
            | Ok _ -> failwith "Impossible case"
            | Error e -> Error ("map4 -> " ^ e)))

  let map5 mapper (Decoder decoder1) (Decoder decoder2) (Decoder decoder3)
      (Decoder decoder4) (Decoder decoder5) =
    Decoder
      (fun value ->
        let open Tea_result in
        match
        ...
        | e1, e2, e3, e4, e5 -> (
            let open! Tea_result in
            match e1 |> first e2 |> first e3 |> first e4 |> first e5 with
            | Ok _ -> failwith "Impossible case"
            | Error e -> Error ("map5 -> " ^ e)))

  let map6 mapper (Decoder decoder1) (Decoder decoder2) (Decoder decoder3)
      (Decoder decoder4) (Decoder decoder5) (Decoder decoder6) =
    Decoder
      (fun value ->
        let open Tea_result in
        match
        ...
        | e1, e2, e3, e4, e5, e6 -> (
            let open! Tea_result in
            match
              e1 |> first e2 |> first e3 |> first e4 |> first e5 |> first e6
            with
            | Ok _ -> failwith "Impossible case"
            | Error e -> Error ("map6 -> " ^ e)))

  let map7 mapper (Decoder decoder1) (Decoder decoder2) (Decoder decoder3)
      (Decoder decoder4) (Decoder decoder5) (Decoder decoder6)
      (Decoder decoder7) =
    Decoder
      (fun value ->
  ...
  let map8 mapper (Decoder decoder1) (Decoder decoder2) (Decoder decoder3)
      (Decoder decoder4) (Decoder decoder5) (Decoder decoder6)
      (Decoder decoder7) (Decoder decoder8) =
    Decoder
      (fun value ->
        let open Tea_result in
        match
  ...
  let succeed v = Decoder (fun _value -> Tea_result.Ok v)
  let fail e = Decoder (fun _value -> Tea_result.Error e)
  let value = Decoder (fun value -> Tea_result.Ok value)
  ...
  let nullable decoder = oneOf [ null None; map (fun v -> Some v) decoder ]

  (* Decoders *)

  (* TODO:  Constrain this value type more *)
  let decodeValue (Decoder decoder) value =
    try decoder value with
  ...
  let decodeString decoder string =
    ...
    | _ ->
      Tea_result.Error "Invalid JSON string"
end
